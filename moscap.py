...
def make_moscap(device, interface_siox, region_ox, region_si, contact_ox, contact_si, tox, tsi, ox_material, si_material):
    ...
    if False:
        ...
        ds.add_1d_contact  (mesh="moscap", name=contact_ox,    tag="d0", material="metal")
        ds.add_1d_contact  (mesh="moscap", name=contact_si,    tag="d2", material="metal")
        ds.add_1d_interface(mesh="moscap", name=interface_siox, tag="d1")
        ds.add_1d_region   (mesh="moscap", material=ox_material, region=region_ox, tag1="d0", tag2="d1")
        ds.add_1d_region   (mesh="moscap", material=si_material, region=region_si, tag1="d1", tag2="d2")
    else:
        ds.add_1d_mesh_line(mesh="moscap", pos=-tox,   ps=1e-7, tag="d0")
