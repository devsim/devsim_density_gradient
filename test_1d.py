...
def debug_plot(device, region):
    nodes = numpy.array(ds.get_node_model_values(device=device, region=region, name='node_index'))
    node_volume = numpy.array(ds.get_node_model_values(device=device, region=region, name='NodeVolume'))
    num_nodes = len(nodes)
    flux_term     = numpy.zeros(num_nodes)
    edge_vol_term = numpy.zeros(num_nodes)
    x             = abs(numpy.array(ds.get_node_model_values(device=device, region=region, name="x")))*1e7
    node_vol_term = numpy.array(ds.get_node_model_values(device=device, region=region, name="volume_term"))
    pos_edge = []
    neg_edge = []
    ...
    n0_indexes = ds.get_edge_model_values(device=device, region=region, name='node_index@n0')
    n1_indexes = ds.get_edge_model_values(device=device, region=region, name='node_index@n1')
    ...
    ft = ds.get_edge_model_values(device=device, region=region, name="surface_term")
    st = ds.get_edge_model_values(device=device, region=region, name="edge_volume_term")
...
for index, d in enumerate(dopings):
    ...
    ds.set_parameter(name=GetContactBiasName("top"), value=0.0)
    print(d)
    ds.set_parameter(device=device, region=region_si, name="Gamman", value=10)
    ds.set_parameter(device=device, region=region_si, name="Gammanox", value=1)
    ds.set_parameter(device=device, region=region_si, name="Gammap", value=0.1)
    ds.set_parameter(device=device, region=region_si, name="Gammapox", value=0.00001)
...
if carrier_var == 'Electrons':
    ds.edge_model(device=device, region=region_si, name="surface_term", equation="del_log_n1*EdgeCouple")
    ds.edge_model(device=device, region=region_si, name="edge_volume_term", equation="del_log_n2*EdgeNodeVolume")
    ds.node_model(device=device, region=region_si, name="volume_term", equation="Le_eqn*NodeVolume")
elif carrier_var == 'Holes':
    ds.edge_model(device=device, region=region_si, name="surface_term", equation="del_log_p1*EdgeCouple")
    ds.edge_model(device=device, region=region_si, name="edge_volume_term", equation="del_log_p2*EdgeNodeVolume")
    ds.node_model(device=device, region=region_si, name="volume_term", equation="Lh_eqn*NodeVolume")
else:
...
#  plt.show()
#
x=abs(numpy.array(ds.get_node_model_values(device=device, region=region_si, name='x')))*1e7
for i in range(len(dopings)):
    fig = plt.figure()
    plt.plot(
      x,
