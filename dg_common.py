...
def set_dg_parameters(device, region):
    ...
    ds.set_parameter(device=device, region=region, name="h", value=6.62606876e-34)  # J*s
    ds.set_parameter(device=device, region=region, name="hbar", value=1.054571596e-34) # J*s
    ...
    ds.set_parameter(device=device, region=region, name="mass_electron", value=9.10938188e-31) # kg
    ds.node_model(device=device, region=region, name="meff_elec", equation="1.0903*mass_electron")
    ds.node_model(device=device, region=region, name="meff_hole", equation="1.1525*mass_electron")
    ds.node_model(device=device, region=region, name="b_n", equation="(1e4*Gamman*hbar^2)/(q*6*meff_elec)") # eV*cm^2
    ds.node_model(device=device, region=region, name="b_p", equation="(1e4*Gammap*hbar^2)/(q*6*meff_hole)") # eV*cm^2
    # Oxide
    #
    #ds.node_model(device=device, region=region, name="b_nox", equation="1e-100") # eV*cm^2
    ds.node_model(device=device, region=region, name="b_nox", equation="1e4*Gammanox*hbar^2/(6*q*0.14*mass_electron)") # eV*cm^2
    ds.node_model(device=device, region=region, name="b_pox", equation="1e4*Gammapox*hbar^2/(6*q*1.0*mass_electron)") # eV*cm^2
    ...
    ds.node_model(device=device, region=region, name="x_np", equation="1e2*hbar/(2*0.4*mass_electron*q*3.15)^0.5") # cm
    ds.node_model(device=device, region=region, name="x_pp", equation="1e2*hbar/(2*0.4*mass_electron*q*4.50)^0.5") # cm
    ...
    for e in em:
        ds.edge_average_model(device=device, region=region, node_model=e[0], edge_model=e[1], average_type=e[2])
        for d in e[3]:
            ds.edge_average_model(device=device, region=region, node_model=e[0], edge_model=e[1], average_type=e[2], derivative=d)
...
def setup_atox(device, region):
    ...
    for i in ds.get_interface_list(device=device):
        ...
        if region in rlist:
            if rlist[0] == region:
                ds.interface_model(device=device, interface=i, name="iindex", equation="node_index@r0")
            else:
                ds.interface_model(device=device, interface=i, name="iindex", equation="node_index@r1")
            nlist = ds.get_interface_model_values(device=device, interface=i, name="iindex")
            for j in nlist:
                print(j)
                ds.set_node_value(device=device, region=region, index=int(j), name="AtOx", value=1.0)
...
def setup_dg_edge(device, region, carrier_name, parameter_name, variables):
    ...
    ds.edge_model(device=device, region=region, name=model_name, equation=expression)
    for v in variables:
        m0 = ":%s@n0" % (v,)
        m1 = ":%s@n1" % (v,)
        ds.edge_model(device=device, region=region, name=(model_name + m0), equation=(expression + m0))
        ds.edge_model(device=device, region=region, name=(model_name + m1), equation=(expression + m1))
...
def setup_edge_volume(device, region):
    ...
    if dim == 1:
        ds.edge_model(device=device, region=region, name="EdgeNodeVolume", equation="0.5*EdgeCouple*EdgeLength")
    elif dim == 2:
        ds.edge_model(device=device, region=region, name="EdgeNodeVolume", equation="0.25*EdgeCouple*EdgeLength")
    elif dim == 3:
        ds.edge_model(device=device, region=region, name="EdgeNodeVolume", equation="EdgeCouple*EdgeLength/6.0")
    else:
        raise NameError("Unhandled dimension %d" % (dim))
    ds.set_parameter(device=device, region=region, name="edge_node0_volume_model", value="EdgeNodeVolume")
    ds.set_parameter(device=device, region=region, name="edge_node1_volume_model", value="EdgeNodeVolume")
...
def setup_dg_equation(device, region, e_name, v_name, e_m, e_v_m, n_m):
    ds.equation(device=device, region=region, name=e_name, variable_name=v_name,
